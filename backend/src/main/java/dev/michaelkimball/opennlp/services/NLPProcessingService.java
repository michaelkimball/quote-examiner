...
public class NLPProcessingService {
    ...
    public void processParagraph(String paragraph, String id){
        User user = userService.getUserById(id).orElseThrow(UserService.UserNotFound::new);
    ...
    public void bootstrap(){
        if(appConfig.hasBootstrap()) {
            User user = new User();
            user.setId("test-user01");
            user.setName("Test User 01");
